...
public class Flight {
	...
	public void printSeats() {
		for (int i = 0; i < aircraft.row; i++) {
			for (int j = 0; j < aircraft.col; j++) {
				seatMap.put(aircraft.seatLayout[i][j], null);
				System.out.println(aircraft.seatLayout[i][j] + " ");
			}
			System.out.println();
		}
		System.out.println();
	...
	public void reserveSeat(Passenger p) throws Exception {
		...
		if (manifest.contains(p)) {
			throw newe DuplicatePassengerException("Duplicate Passenger" + p.getName() + " " + p.getPassport());
			p.getPassport();
		}

		if (!aircraft.checkSeat(p.getSeat())) {
			throw new SeatOccupiedException("Seat is occupied");
		}
	...
	class DuplicatPassengerException extends Exception {
		punlic DuplicatePassengerException(String errorMessage) {
			super(errorMessage);
		}
	}
	...
	class PassengerNotInManifestException extends Exception {
		public PassengerNotInManifestException(String errorMessage)P
			super(errorMessage);
	}
